...
public class Robot extends TimedRobot {
 ...
 static final int BTNSHOOTER = 3;
  ...
  public void robotInit() {
    ...
    // color sensor defining the matching color 
  ...
  public void robotPeriodic() {
     ...
     //useful if outputting the color to an RGB LED or similar.                          \\
  ...
  //*******************************************************************\\
  // This function is used to read joystick & eliminate deadzone issue/s\\
  //*******************************************************************\\
  ...
  public void ballControl() {
    ...
    if (control_stick.getRawButtonPressed(BTNINTAKE) == true ) {
     ...
     if (control_stick.getRawButtonReleased(BTNINTAKE) == true){
      m_LIntakeMotor.set(0);  //This section reverses the spin to shoot the balls
      m_RIntakeMotor.set(0); 
      }
    
    if (control_stick.getRawButtonPressed(BTNSHOOTER) == true) {
      m_LIntakeMotor.set(m_LOutake);  //This section reverses the spin to shoot the balls
...
//while button 0 is held the color sensor will do the following lines of code
  if (button>=0){
    ...
    if(detected.color == kBlueTarget){
      blueTemp = blueCount;
      m_RIntakeMotor.set(0.30);
      m_LIntakeMotor.set(-0.30);
    }
    ...
    total = yellowTemp+redTemp+blueTemp+greenTemp;
  ...
  if(rotation>6 && detected.color == kRedTarget && button==0){
    m_RIntakeMotor.set(0.0);
    m_LIntakeMotor.set(0.0);
      total = endtotal;
        }
