...
//////////////////////////BUTTON MAPPINGs\\\\\\\\\\\\\\\\\\\\\\\\\
 ...
 // static final int endtotal = 6;
  ...
  Spark m_ColorSpinner  = new  Spark(4);
  ...
  public void autonomousPeriodic() {
    ...
    } while(timer.get()>=1 && timer.get()<5){
      m_BeltMotor.set(0.9);
    }
    while(timer.get()>=5&&timer.get()<7){
      m_BeltMotor.set(0.0);
      m_LOutakeMotor.set(0.0);
      m_ROutakeMotor.set(0.00);
      m_left.set(0.4);
      m_right.set(-0.4);
    }
   ...
   while(timer.get()>=8&&timer.get()<10){
    m_left.set(0.3);
    m_right.set(-0.3);
  }
  m_left.set(0.0);
...
*/
  ...
  public void teleopPeriodic() {
    ...
    if (drive_stick.getRawButtonPressed(5) == true) {
      switchedCamera.setSource(camera1);
    } else if( drive_stick.getRawButtonPressed(6)) {
  ...
  */
  ...
  public void ballControl() {
    ...
    if (control_stick.getRawButtonReleased(BTNSHOOTERBACK) == true) {
      m_LOutakeMotor.set(0); //This section defines the high speed shot
      m_ROutakeMotor.set(0);
    }
  }
  public void Climber() {
...
//while button 0 is held the color sensor will do the following lines of code
 ...
 /* public void updateDisplays() {
    if (displayCtr % 25 == 0) { // Update displays on Dashboard every ~500msec
      ...
      SmartDashboard.aputString("DB/String 2", "Current Stage: " + Double.toString(currentStage));
