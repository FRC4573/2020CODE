...
// static final int BTNINTAKE = 1;  // use trigger for intake
 //static final int BTNSHOOTER = 3; 
  ...
  //static final variables to set counts for colors
...
// figure out which way to enable lift
  ...
  */
    // ***************************************************************************
  // This function controls the ball motor with color sensor = green
  // ***************************************************************************  
...
*/
  //  ballMotorSpeed = 0;
   ...
   //color.match == ktargetgreen
   int colorNum = m_colorSensor.getBlue();
  ...
  //code to be fixed // make go no to yes
 // int total = 0;
  if (control_stick.getRawButton(BTNSTAGE0) == true){
    for(rotation = 0 ; rotation <= 3;){
    ...
    if(detected.color== kYellowTarget){
...
System.out.println(total);
  ...
  /*  if (greenCount<3 && proximity>400 && proximity<1000){
      m_ballMotor.set(.3);
    */
  ...
  /*      while( greenCount<3 && detected.color==kGreenTarget){
        greenCount++;
        System.out.println(greenCount);
      }
    }
    //if(greenCount>=3 && detected.color ==kGreenTarget){
      //m_ballMotor.stopMotor();
    //}
  */
 /* good code 
  if (detected.color!= kGreenTarget && proximity>400 && proximity<2047){
      m_ballMotor.set(.3);
    }
    else{
      m_ballMotor.stopMotor();
